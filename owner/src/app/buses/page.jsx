...
export default function BusesPage() {
  ...
  const { data: busRoutesData, isLoading: routesLoading } =
    useGetBusRoutesQuery(
      { busId: selectedBusId },
      { skip: !selectedBusId || !showRouteEditor }
    );
  const [updateBusRoutes, { isLoading: isUpdatingRoutes }] =
    useUpdateBusRoutesMutation();
  ...
  useEffect(() => {
    if (busRoutesData?.routes && showRouteEditor) {
      setEditingRoutes(
        busRoutesData.routes.map((route) => ({
          routeId: route._id,
              ...
              {isLoading
                ...
                : paginatedBuses.map((bus) => (
                    <BusCard
                      ...
                      handleEditRoutes={handleEditRoutes}
                      deleteBus={deleteBus}
                      setShowBulkForm={setShowBulkForm}
                      setEditMode={setEditMode}
                      setSelectedBusId={setSelectedBusId}
                      handleOpenBulkForm={handleOpenBulkForm}
                      setBulkBusData={setBulkBusData}
                      setShowForm={setShowForm}
                      handleOpenBusForm={handleOpenBusForm}
                  ...
                  {editingRoutes.map((route, routeIndex) => (
                    ...
                    >
                      <h3 className="text-lg font-semibold text-gray-800 mb-4">
                        Route {routeIndex + 1}: {route.routeFrom} →{" "}
                        {route.routeTo}
                      </h3>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <DatePicker
                            ...
                            onChange={(time) =>
                              handleRouteChange(routeIndex, "arrivalTime", time)
                            }
                            showTimeSelect
                            showTimeSelectOnly
                            timeIntervals={15}
