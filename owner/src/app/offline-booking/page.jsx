...
const OfflineBookingPage = () => {
  ...
  // Payment handlers
  const { processPayment, handlePaymentSuccess, handlePaymentFailure } = usePaymentHandlers(
    createBookingMutation,
    async (paymentData) => {
      // This will be handled by the backend confirm payment endpoint
      const response = await fetch('http://localhost:9090/api/v1/bus-owner/offline-booking/confirm-payment', {
        method: 'POST',
        headers: {
